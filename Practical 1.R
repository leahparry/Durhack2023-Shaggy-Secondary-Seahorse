f <- function(x) sin(2*pi*x) #define actual computer model
...
#this is evaluated at the point x = 0.25
...
em_out_1pt
# this agrees with above manual calculation of this value
...
em_out <- matrix(0,nrow=nP,ncol=2,dimnames=list(NULL,c("ExpD_f(x)","VarD_f(x)")))    # set up matrix for results
...
head(em_out)
#we are unneccsarily repeating calculations every single emulator run so will fix that later

########### plot emulator output #############
# pdf(file="fig1A_sin_emulator.pdf",height=7,width=8.5)    # uncomment if you want a pdf file
plot(xP,em_out[,"ExpD_f(x)"],ylim=c(-1.2,1.2),ty="l",col="blue",lwd=2.5,xlab="Input parameter x",ylab="Output f(x)") #plots expectation
lines(xP,em_out[,"ExpD_f(x)"]+3*sqrt(em_out[,"VarD_f(x)"]),col="red",lwd=2.5)
lines(xP,em_out[,"ExpD_f(x)"]-3*sqrt(em_out[,"VarD_f(x)"]),col="red",lwd=2.5)
#plots prediction interval
...
#dev.off() #uncomment if you want pdf
